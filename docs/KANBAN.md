...
Related: [PROJECT_PLAN.md](./PROJECT_PLAN.md) · [PLAN_PROYECTO.md](./PLAN_PROYECTO.md)
...
**Acceptance criteria / Criterios de aceptación**
- [ ] User stories written for upload, detect, metrics, report, deploy
- [ ] Definition of done agreed by team (30-min meeting)
- [ ] Kanban board created and shared
- [ ] Scope document: no more than 2 brands
...
**Description (EN)**  
Prepare technical presentation: problem → solution → architecture → live demo on deployed URL → tech stack → limitations → future work. Assign speaking parts to A, B, C. Run 15-min dry-run on Day 5 and final rehearsal on Day 6. SM closes Kanban, tags release `v1.0-demo`.

**Descripción (ES)**  
Preparar presentación técnica: problema → solución → arquitectura → demo en URL desplegada → stack → limitaciones → trabajo futuro. Asignar partes a A, B, C. Ensayo de 15 min el Día 5 y ensayo final el Día 6. SM cierra Kanban y tag `v1.0-demo`.
...
| **Day 2** | ISSUE-04, 05 (finish), 06, 07, 15 (start) |
| **Day 3** | ISSUE-08, 09 |
...
1. Create project → Board view  
2. Columns: Backlog · To Do · In Progress · In Review · Done  
3. Create one issue per `ISSUE-XX` above (copy title + description)  
4. Assign labels: `day-1` … `day-6`, `ml`, `backend`, `frontend`, etc.  
5. Assign responsible person (A, B, or C)

---

_Last updated: sprint start / actualizar al inicio del sprint_
